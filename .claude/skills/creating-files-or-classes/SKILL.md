---
name: creating-files-or-classes
description: When creating files or classes follow the instructions explained here
---

When creating any file or class (composables, ViewModels, repositories, etc.), always take into account these guidelines:

1. **Keep files short**: Limit file length to ~200 lines (with a maximum of +50 lines spare). Import statements do not count toward this limit.
2. **Split when approaching the limit**: If a file is approaching the limit, subdivide it into child classes or subfiles with focused responsibilities.
3. **Keep names simple and self-describing**: A class or file name should clearly express its single purpose without needing a comment to explain it.