...
- `ClienteRepositoryImpl.delete(id)` soft-deletes the cliente **and its pedidos**, then enqueues one `CLIENTE` op — the Supabase `trg_clientes_cascade_soft_delete` trigger flips the pedidos server-side for every other device (see `docs/db-schema.md` §8). It currently has **no UI caller**: clientes cannot be deleted from any screen today (blacklisting is the closest thing). The cascade is implemented anyway so it cannot drift from the mercado path.
- `ClienteDao.countByMercado(mercadoId)` counts live clientes of a mercado, blacklisted included — it backs the "también se eliminarán N clientes" warning in the mercado delete dialog.
